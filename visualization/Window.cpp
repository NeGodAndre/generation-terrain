...
bool Window::Create(const DescWindow &desc)
{
	...
	long lwidth = (long)m_desc.width;
	long lheight = (long)m_desc.height;

	long lleft = (long)m_desc.posx;
	long ltop = (long)m_desc.posy;

	m_hwnd = CreateWindowEx(NULL, L"D3D11", m_desc.caption.c_str(), WS_OVERLAPPEDWINDOW | WS_VISIBLE, lleft, ltop, lwidth, lheight, NULL, NULL, NULL, NULL);
...
LRESULT Window::WndProc(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE: case WM_LBUTTONUP: case WM_LBUTTONDOWN: case WM_MBUTTONUP: case WM_MBUTTONDOWN: case WM_RBUTTONUP: case WM_RBUTTONDOWN: case WM_MOUSEWHEEL: case WM_KEYDOWN: case WM_KEYUP:
		DirectX::Mouse::ProcessMessage(nMsg, wParam, lParam);
		DirectX::Keyboard::ProcessMessage(nMsg, wParam, lParam);
		if (m_inputmgr)
...
void Window::m_UpdateWindowState()
{
	...
	ClientRect.right = m_desc.posx + m_desc.width;
	ClientRect.bottom = m_desc.posy + m_desc.height;
	if (m_inputmgr)
